...
class ProviderAuthController extends Controller
{
    public function __construct() {
        $this->middleware('auth:provider-api', ['except' => ['login', 'register','index','checkcode','forget','code']]);
    }
    ...
    public function login(Request $request){
        ...
        if (!$token = auth()->guard('provider-api')->attempt($validator->validated())) {
        ...
        if (Provider::where('email', $request->email)->exists()){
            ...
            if($status=='inactive') {
                ...
                $otp = Provider::where('email', $request->email)
                    ->update(['code' => $code]);
    ...
    public function checkcode(Request $request)
    {
        ...
        if ($result==true) {
            ...
            if ($request->code == $code) {
                ...
                $token= auth()->guard('provider-api')->login($check);
    ...
    public function me()
    {
        return response()->json(auth('provider-api')->user());
    }
    ...
    public function update_profile(Request $request){
        $validator = Validator::make($request->all(),[
                ...
                'phonenumber'=>'required',
                'email' => 'required|email|unique:providers,email,' .auth('provider-api')->user()->id,
            ]
            ,[
    ...
    public function code(Request $request)
    {
        ...
        if(ResetCodePassword::where('code', $request->code)->exists()){
            ...
            if ($result == true) {
                ...
                // find user's email
                $user = Provider::firstWhere('email', $passwordReset->email);
